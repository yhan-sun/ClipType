...
class ClipTypeLocalizations {
  ...
  String get codeModeSubtitle => _text(
    'Code mode uses keyboard actions: it skips leading indentation, types opening delimiters and quotes, and moves right over matching closers already generated by the editor. Content inside strings and comments is kept; typo simulation is disabled. Editor auto-pair must be enabled.',
    'Code 模式走键盘事件：跳过每行行首缩进，输入开括号和引号，并用右方向键跳过编辑器已生成的对应闭括号或引号。字符串和注释中的内容会保留，且不使用错字模拟；需要开启编辑器自动补全。',
  );
