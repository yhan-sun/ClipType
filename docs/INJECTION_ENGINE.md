...
- `code`: explicit keyboard path for source code. It skips leading indentation,
  types opening delimiters and quotes, and uses cursor-right actions to pass
  over matching pairs already generated by the destination editor. Recognized
  strings and comments remain literal; no clipboard revision witness is used.
