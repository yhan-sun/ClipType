...
action to pass over matching pairs already generated by the editor. Delimiters
inside recognized strings and comments remain literal. Code mode assumes the
destination editor's auto-pair and auto-indent behavior is enabled; it does not
read target content or claim editor-specific formatting semantics.
