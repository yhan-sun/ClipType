...
class ClipTypeLocalizations {
  ...
  String text(String english, String chinese) => _text(english, chinese);
  ...
  String get codeModeSubtitle => _text(
    'Code mode is keyboard-only. It cooperates with editor auto-pair and auto-indent for (), {}, [], double quotes, and single quotes. Triple quotes and backticks are typed explicitly. Safe corrected-typo simulation can be enabled for ordinary ASCII source characters; brackets, quotes, slash, navigation, and non-ASCII text are excluded.',
    '代码模式仅使用键盘事件，并与编辑器对 ()、{}、[]、双引号和单引号的自动配对/自动缩进协作。三引号与反引号会显式输入。可以为普通 ASCII 源字符启用安全的“错字后立即纠正”模拟；括号、引号、斜杠、导航动作和非 ASCII 文本不会参与。',
  );
  ...
  String get typoWarning => _text(
    'Corrected typos are optional and immediately repaired. Keep this off for passwords, terminals, commands, checksums, or other exact-data entry.',
    '纠错错字为可选模拟，并会立即修正。密码、终端、命令、校验和或其他精确数据录入建议保持关闭。',
  );
