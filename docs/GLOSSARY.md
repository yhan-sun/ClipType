...
**Code mode** — Explicit keyboard mode for source code and structured text. It
skips leading indentation, types opening delimiters and ordinary quotes, and
uses cursor-right actions to pass over matching pairs already generated by the
destination editor. Python-style triple-quoted boundaries are typed
explicitly.
