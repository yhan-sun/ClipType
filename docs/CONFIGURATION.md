...
- `"code"` — keyboard code actions for source code and structured text;
...
Code mode requires the keyboard, line-break, Tab, Unicode, modifier-observation,
and cursor-right capabilities as applicable to the source. It skips leading
spaces and Tabs at each normal-code line, types opening delimiters and quotes, and moves right over matching
closers or ordinary quotes already generated by an editor with auto-pair
enabled. Python-style triple-quoted boundaries (`"""` and `'''`) are typed
explicitly at both ends and are never skipped with cursor-right. Markdown
triple-backtick fences are also typed literally and do not change pair
handling inside the fence.
Delimiters inside recognized strings and comments remain literal. It does not
use Clipboard paste or revision guarding, and corrected typo simulation is
